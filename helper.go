...
const (
	ttsDateFormat = "Mon Jan 02 2006 15:04:05 GMT-0700 (Coordinated Universal Time)"
	ssmlTpl       = "<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xml:lang='en-US'><voice name='%s'><prosody pitch='+0Hz' rate='%s' volume='%s'>%s</prosody></voice></speak>"
)
...
func mkSSML(text interface{}, voice string, rate string, volume string) string {
	...
	ssml := fmt.Sprintf(ssmlTpl, voice, rate, volume, textStr)
	return ssml
}
...
func dateToString() string {
	return time.Now().UTC().Format(ttsDateFormat)
}
